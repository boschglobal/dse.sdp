	...
	testscript.Run(t, testscript.Params{
		...
		Cmds: map[string]func(ts *testscript.TestScript, neg bool, args []string){
			...
			"yamlcontains": func(ts *testscript.TestScript, neg bool, args []string) {
				...
				if len(args) == 2 {
					// Check the ypath returned an object.
					var value any
					err = path.Read(strings.NewReader(got), &value)
					if err == nil {
						// Match
						if neg {
							ts.Fatalf("yamlcontains %q; %q path found in file (unexpected)", args[0], ypath)
						}
					} else {
						// No_Match
						if !neg {
							ts.Fatalf("yamlcontains %q; %q path not found in file", args[0], ypath)
						}
					}
				} else {
					// Check the want matches.
					want := args[2]
					var value string
					err = path.Read(strings.NewReader(got), &value)
					if err != nil {
